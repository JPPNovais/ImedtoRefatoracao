#!/usr/bin/env bash
#
# dev.sh — sobe o ambiente de dev local apontando para o banco da EC2.
#
...
# Parar: Ctrl+C
#
# A cada execução REINICIA: mata qualquer instância anterior (túnel/backend/front
# aberta em outro terminal) e sobe tudo do zero.
...
# ── 3. Frontend Vite (foreground) ────────────────────────────────────────────
[ -d "$ROOT/frontend/node_modules" ] || { say "Instalando deps do front…"; ( cd "$ROOT/frontend" && npm install ); }

echo
ok "${B}Ambiente pronto${N}"
echo "   ${B}Frontend${N}  http://localhost:$FRONTEND_PORT"
