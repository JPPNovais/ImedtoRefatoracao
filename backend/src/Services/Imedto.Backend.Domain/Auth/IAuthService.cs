...
/// <summary>
/// Contrato de autenticação. Implementado na Infrastructure por <c>LocalJwtAuthService</c>.
/// Injetado diretamente nos controllers — não passa pelo bus CQRS (auth não é comando de domínio).
/// </summary>
public interface IAuthService
{
    /// <summary>
    /// Cria uma nova conta. Retorna o <see cref="SignupResult"/>:
    /// - Se a configuração de "Email confirmations" estiver desligada, vem uma sessão ativa pronta pra setar cookies.
    /// - Se ligada, a sessão vem nula e o usuário precisa confirmar o e-mail antes de fazer login.
    /// </summary>
    ...
    /// ela é criada automaticamente. O <see cref="ConviteResult.ActionLink"/>
    ...
    /// <summary>Revoga o refresh token associado à sessão. Fire-and-forget — não lança exceção.</summary>
    ...
    /// <summary>Exclui a credencial de auth (LGPD — direito ao esquecimento).</summary>
