...
<template>
    <div class="catalog-page">
        <div class="page-header">
            <div>
                <h1 class="page-titulo">Regiões anatômicas globais</h1>
                <p class="page-subtitulo">Catálogo de regiões anatômicas disponíveis para o exame físico.</p>
            </div>
            <button class="btn-primario" @click="irParaForm()">+ Nova região</button>
        </div>
            ...
            <table class="tabela">
                ...
                <tbody>
                    ...
                    <tr v-if="store.lista.length === 0">
                        <td colspan="6" class="estado-centro">Nenhuma região encontrada.</td>
                    </tr>
                </tbody>
            </table>
        ...
        <div v-if="modalAcao" class="modal-overlay" @click.self="fecharModal">
            <div class="modal">
                <h2 class="modal-titulo">{{ acaoTipo === "desativar" ? "Desativar região" : "Reativar região" }}</h2>
...
.input-busca { flex: 1; min-width: 200px; padding: 7px 10px; border: 1px solid hsl(var(--border)); border-radius: 6px; font-size: 13px; background: hsl(var(--background)); color: hsl(var(--foreground)); }
...
.col-sinonimos { max-width: 200px; }
.col-acoes { display: flex; gap: 6px; }
.sinonimos-lista { display: flex; flex-wrap: wrap; gap: 4px; align-items: center; }
.tag-sinonimo { display: inline-block; padding: 1px 6px; border-radius: 4px; font-size: 11px; background: hsl(var(--muted)); color: hsl(var(--muted-foreground)); }
.texto-muted { color: hsl(var(--muted-foreground)); font-size: 12px; }
