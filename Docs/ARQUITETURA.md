...
- **Vista circunferencial (briefing 2026-06-08_005 B1, atualizado 2026-06-25_002)**: 7 nós agregadores nível-1 (`{base}-circunferencial`) existem no catálogo (cabeça, pescoço, tronco, e os 4 membros). Esses nós não têm filhos próprios nem `svg_coords`. A modal resolve os filhos via `RAMOS_CIRCUNFERENCIAL[idCirc]` → `{anterior, posterior}` — getFilhos dos dois ramos. **Tronco circunferencial (fusão 2026-06-25_002)**: `tronco-circunferencial` é simétrico: ramo anterior = `tronco-anterior`, ramo posterior = `tronco-posterior`. Não há exceção clínica (a exceção abdome↔lombossacra foi removida junto com as sub-regiões torax/abdome/pelve). O `RegiaoTreeView` do admin exibe um terceiro grupo de vista `circunferencial`.
- **Fusão estrutural do tronco (briefing 2026-06-25_002)**: as 9 sub-regiões `torax/abdome/pelve × anterior/posterior/circunferencial` foram removidas do catálogo. O tronco passa a ter apenas 3 regiões nível-1 reais: `tronco-anterior`, `tronco-posterior`, `tronco-circunferencial`. `tronco-anterior` e `tronco-posterior` têm hotspot real no `BodyMap` (entradas em `bodyMapPaths.ts`). Não existe mais pseudo-hotspot sintético nem `PARTE_PARA_TRONCO` (estrutura removida de `regioesCircunferenciais.ts`). O `BodyMap` renderiza tronco via `regioesComPath` como qualquer outra região — o evento `troncoClicado` foi removido; apenas `regiaoClicada` existe.

**Cache de leitura reforçado + invalidação (briefing 2026-06-25_001):** o exame físico consome o catálogo via `GET /api/catalogo/regioes-anatomicas` (`ListarRegioesCatalogoQueryHandlers`), que cacheia o resultado em `IMemoryCache` com **TTL longo** (≥6h — chave `catalogo:regioes:vista={...}:ativas={...}`, varia por `vista` e `ApenasAtivas`) porque o payload é pesado (>100 linhas com JSON de coordenadas) e o catálogo raramente muda. A paridade com o admin é garantida por **invalidação**, não pelo TTL, em duas camadas: **(a) automática** — os 5 handlers admin de mutação de região (`Criar`/`Atualizar`/`Inativar`/`Reativar`/`Excluir`RegiaoAdminCommandHandler`) invalidam `catalogo:regioes:*` após persistir; **(b) manual** — endpoint admin `POST /api/admin/catalogos/regioes-anatomicas/invalidar-cache` (policy `ImedtoAdmin`), acionado por um botão "Forçar atualização do exame físico" no configurador, como reforço. Ambas compartilham o mesmo invalidador e cobrem **todas as variações de chave**. Toda edição (nome, template, vista, lateralidade, ativo/inativo, hierarquia) repercute no exame físico **na próxima abertura**. Mesmo padrão do `IConfigGlobalReader` ("invalida cache após mutação"). O `BodyMap` casa o polígono ↔ região por **código** (não por nome), de modo que renomear uma região nível-1 no admin não a remove do desenho.
